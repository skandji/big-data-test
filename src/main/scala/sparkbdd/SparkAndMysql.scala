...
object SparkAndMysql {
  ...
  def testConnectionDbMysql(sparkSession: SparkSession) = {
    ...
    val dfMysql = readDataFrameFromMySqlDb(sparkSession, "localhost:3361", "spark_order_db","orders", "root",
    "PassWord23!")
    if (dfMysql != null) {
      dfMysql.printSchema()
    ...
    val dfQuery = readDataFrameFromMySqlDb(sparkSession, "localhost:3361",
      "spark_order_db",
      "(select state, city, sum(round(numunits * totalprice)) as commandes_totales from spark_order_db.orders group by state, city) table_summary",
      "root",
      "PassWord23!")
    if (dfQuery != null) {
  ...
  def joinTables(sparkSession: SparkSession) = {
    /*val dfOrders = sparkSession.read.jdbc("jdbc:mysql://127.0.0.1:3361/spark_order_db",
      "spark_order_db.orders", properties)*/

    val dfOrders = readDataFrameFromMySqlDb(sparkSession, "localhost:3361", "spark_order_db","orders", "root",
      "PassWord23!")

    val dfOrderLine = readDataFrameFromMySqlDb(sparkSession, "localhost:3361", "spark_order_db","orderline", "root",
      "PassWord23!")
